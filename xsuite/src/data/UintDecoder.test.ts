...
describe("UintDecoder success test cases", () => {
  ...
  successTestCases.forEach(([bytes, value, topEncoding, nestedEncoding]) => {
    test(`Bytes: ${bytes}, Value: ${value}, Top-decode`, () => {
      expect(new UintDecoder(bytes).topDecode(topEncoding)).toEqual(value);
    });
    test(`Bytes: ${bytes}, Value: ${value}, Nest-decode`, () => {
      expect(new UintDecoder(bytes).nestDecode(nestedEncoding)).toEqual(value);
    });
  });
});
