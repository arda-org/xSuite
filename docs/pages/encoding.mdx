...
### List
- Encodes an array of values. Uses nest encoding for the underlying data. Adds the length of the array at the beginning when it is itself nest encoded i.e. as part of a `Tuple`.
```ts
...
The above data types can be combined to pass complex data as part of a contract call or query:
```ts
...
const { returnData } = await world.query({
  callee: contract,
  funcName: "positions",
  funcArgs: [e.Addr(address), e.Str('TOKEN-123456')],
});
