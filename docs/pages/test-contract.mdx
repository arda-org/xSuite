...
Here, using `assertAccount`, we will test that the contract we deployed has the correct balance and storage we expect. `contract.getAccountWithKvs()` will fetch the account of the contract with all its kvs (short for "key-value pairs") from the simulnet.

Using `hasKvs` we can test that the contract contains the kvs provided (use `allKvs` if you want to test that the contract contains exactly all the kvs provided).
...
- `allKvs` - will check if the account has these EXACT kvs we want, if it has extra kvs the assertion will fail (this is similar to how MultiversX Scenarios work)

You can use whichever works best for you. We recommend using `allKvs` to make sure your contract doesn't have any unwanted kvs, but sometimes it may be easier to use `hasKvs`, when there are a lot of storage or when debugging.
