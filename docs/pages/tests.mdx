...
<Callout type="warning" emoji="⚠️">
  Due to a current bug in the underlying MultiversX Scenarios library, you should always specify `payable` in the `codeMetadata` of the `setAccount` method in order for payable endpoints to continue to work properly.
</Callout>
