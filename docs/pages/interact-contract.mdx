...
The code for interacting with the real blockchain is nearly the same as the code for testing your contract and interacting with the simulnet. Instead of using a `SWorld` object, you will use a `World` object. That's the only difference.
