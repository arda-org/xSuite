...
Interactions are CLI commands written using the [commander](https://github.com/tj/commander.js) NodeJs library. You should already have the `interact/data.json` and `interact/index.ts` files created after following the getting started guide. Below we will detail the contents of the files.
...
`envChain` is the utility we talked previous and `World` is an wrapper over a real MultiversX blockchain (similar to `FWorld` from tests, which is a wrapper over a "fake" MultiversX blockchain).

We then import the `data.json` file and will use the `e` and `d` utilities from `xsuite/data` to encode/decode data respectively to a format that the MultiversX blockchain understands. More info on the [Data Encoding](/encoding) page
...
1. In the `deploy` command, we first call `loadWallet` to get a new instance of `WorldWallet`, which represents a wallet on the MultiversX blockchain. When calling this function, we will be prompted for the keystore file's password.
...
The command can then be run for devnet like follows:
```
npm run interact:devnet deploy
npm run interact:devnet upgrade
```

Interactions are very similar to tests since they use the same underlying wrappers. For more information on how to deploy contracts, call contracts and query contracts check out the [Contract Operations](/tests#contract-operations) section on the Testing Contracts page.

### Full basic interact file code
...
If you want to create more advanced commands, with advanced options or arguments, it is highly recommended to check out the [commander](https://github.com/tj/commander.js) documentation. However, below we will detail some examples on how to write more advanced interactions.

Suppose you want an interaction that takes some arguments, to easily create new resources for tests. You could do something like this:

```ts
program.command("createOffer")
...
You can call the above command like this for devnet: `npm run interact:devnet createOffer TOKEN-123456 1000`. It will execute the transaction with `token` being `TOKEN-123456` and `amount` being `1000`.
You can execute this command easily with different arguments to quickly make multiple different transactions to the contract for testing purposes, perhaps to set up different cases to check how a DApp UI will look like depending on the contract data.

Of course uou are also not limited to calling only one `callContract` function or `query` function per command, or even deploying only one contract using `deployContract`. You can call the functions multiple times to setup more advanced contracts, like for example if you need to deploy 2 contract which depend on each other:
```ts
...
You can call the above command like this for devnet: `npm run interact:devnet deploy`. The `decimals` argument is optional, and will have a default of `18` if it is not specified.
...
The `World` class represents an abstraction over the MultiversX blockchain, specifically the Gateway API. It provides helper functions to managed accounts, deploy contracts and do transactions. Below are a list of the most relevant functions that the class provides:

- `getAccountBalance(address: Address): Promise<bigint>;` - gets only the alance of an account
...
- `upgradeContract(sender: Signer, txParams: Omit<UpgradeContractTxParams, "sender" | "nonce" | "chainId">): TxResultPromise<CallContractTxResult>;` - allows to upgrade a contract
- `transfer(sender: Signer, txParams: Omit<TransferTxParams, "sender" | "nonce" | "chainId">): TxResultPromise<TxResult>;` - initiate a transfer from a sender to a receiver
- `callContract(sender: Signer, txParams: Omit<CallContractTxParams, "sender" | "nonce" | "chainId">): TxResultPromise<CallContractTxResult>;` - call a contract endpoint
- `query(query: Query): Promise<{ returnCode: string; returnMessage: string; returnData: string[]; }>;` - query a contract view

The functions from above that are related to accounts also have wrappers in the `WorldWallet` and `WorldContract` classes, for easier interaction.
