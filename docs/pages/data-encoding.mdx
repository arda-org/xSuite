...
Under `e.p.Mapper` there are utilities to easily encode storage mappers. This is used together with the `assertAccount` utility from `xsuite` and the `hasPairs`/`allPairs` key, to check that an account has the appropriate storage data.
