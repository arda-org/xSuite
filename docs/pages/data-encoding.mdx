...
The `xSuite/data` package provides the `e` utility to easily convert data to the MultiversX format.
...
Under `e.p.Mapper` there are utilities to easily encode storage mappers. This is used together with the `assertAccount` utility from `xSuite/assert` and the `hasPairs`/`allPairs` key, to check that an account has the appropriate storage data.
...
Under `e.p.Esdts` there are utilities to easily encode ESDTs and assert that an account has the appropriate tokens.
